...
function Posts({ imgUrl, username, caption, comments, id }) {
  ...
  return (
    <div className="post">
      <div className="post_header">
        <Avatar className="post_avatar" alt={username} src={imgUrl} />
        <h3 className="post_header_usernamee">{username}</h3>
      </div>

      <img src={imgUrl} alt="image" className="specific_post" onClick={()=>history.push(`/specific_post/${id}`)} />
        ...
        <div className="post_comments">
          {comments.map((commnt, index) => {
            if (index < 3)
              return (
                <p key={commnt._id}>
                <strong> {commnt?.userWhocomment_name}</strong> :  {commnt?.text} 
                </p>
              );
          })
          }
          {
            comments.length>3?<i className="viewOthercommnt" onClick={()=>history.push(`/specific_post/${id}`)}>View other comments</i>: ""
          }
